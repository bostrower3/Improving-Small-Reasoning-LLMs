...
parser.add_argument('--wandb_project', type=str, default='reasoning-eval')
parser.add_argument('--wandb_entity', type=str, default=None)
parser.add_argument('--wandb_api_key_file', type=str, default='wandb_api.txt')
...
def init_wandb(time_taken):
    run_name = args.run_name or f"{model_path.replace('/', '_')}_{dataset_name.replace('/', '_')}_{tok_limit}"

    if os.path.exists(args.wandb_api_key_file):
        with open(args.wandb_api_key_file) as f:
            key = f.read().strip()
        wandb.login(key=key)
    else:
        print(f"Warning: wandb api key file '{args.wandb_api_key_file}' not found. Wandb logging disabled.")
        return False
    ...
    wandb.init(
        ...
        config={
            ...
            'scale': scale,
            'tok_limit': tok_limit,
            'generation_time_seconds': time_taken,
        }
    )
    return True
...
def get_scores(ds, outputs, save_file_name=None):
    ...
    for input, output in zip(ds, outputs):
        ...
        avg_tokens = sum([len(resp.token_ids) for resp in output.outputs]) / len(output.outputs)
    ...
    print(f"Pass @ 1(with majority): {pass_at_n}")

    final_scores = {
        'pass@1': pass_at_1,
        'pass@1(majority)': pass_at_n,
    ...
    if wandb.run:
        wandb.log({
            ...
            'final_avg_length': sum(tokens) / len(tokens),
            'final_pass_at_1_majority': pass_at_n,
            'final_avg_pass_rate': avg_pass_rate,
        })

        artifact = wandb.Artifact("eval_results", type="dataset")
        if save_file_name is not None and os.path.exists(save_file_name):
...
def evaluate_model(model_name):
    ...
    model = LLM(
        model_name,
        tokenizer=f'deepseek-ai/DeepSeek-R1-Distill-Qwen-{scale}',
    ...
    for x in test_ds:
        ...
        prompt_tokens = hf_tokenizer.apply_chat_template(prompt, add_generation_prompt=True, tokenize=False)
    ...
    sampling_params.stop_token_ids = [hf_tokenizer.eos_token_id]

    print("Generating test outputs...")
    print(hf_tokenizer.decode(hf_tokenizer.encode(test_prompts[0]), skip_special_tokens=False))

    start_time = time.time()
    test_outputs = model.generate(prompts=test_prompts, sampling_params=sampling_params, use_tqdm=True)
    end_time = time.time()
    ...
    test_scores = get_scores(
        test_ds,
        test_outputs,
        f"outputs/{dataset_name.replace('/', '_')}_results_{model_path.replace('/', '_')}_{tok_limit}.json"
    )
